...
// Normal Schedule collection for a team member
// Uses AvailabilityEvent with template dates like 'template-sunday', 'template-monday', etc.
// and isNormalSchedule: true flag
export interface NormalSchedule {
  teamMemberId: string;
  events: AvailabilityEvent[]; // Reuse existing AvailabilityEvent structure
  isEnabled?: boolean; // Whether to show normal schedule on main calendar
  lastUpdated?: string;
}
...
export interface AvailabilityEvent {
  ...
  // Normal Schedule metadata
  isNormalSchedule?: boolean; // True if this is a normal schedule template event
  dayOfWeek?: string; // Day of week for normal schedule events (e.g., 'monday', 'tuesday')
  isExpandedNormalSchedule?: boolean; // True if this is a normal schedule event expanded to a specific date
}
