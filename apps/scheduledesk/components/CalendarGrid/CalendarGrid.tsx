...
const CalendarGrid: React.FC<CalendarGridProps> = ({
  ...
  const renderEvent = (event: AvailabilityEvent) => {
    ...
    // Recurring event classes
    if (event.isRecurring) {
      eventClasses.push('recurring-event');
    }
    if (event.isInstance) {
      eventClasses.push('recurring-instance');
    }
    if (event.recurrence && !event.isInstance) {
      eventClasses.push('recurring-base');
    }

    // Normal schedule event classes
    if (event.isNormalSchedule) {
      eventClasses.push('normal-schedule-event');
    }
    if (event.isExpandedNormalSchedule) {
      eventClasses.push('expanded-normal-schedule-event');
    }
