...
const EventEditor: FC<EventEditorProps> = ({
  ...
  const isSaveable = useMemo(() => {
    ...
    // For normal schedule events, we don't require dates - just event type and custom name
    if (isNormalSchedule) {
      return hasEventType && hasCustomEventName;
    }

    // Basic validation: need event type and start date as minimum for regular events
    return hasEventType && hasCustomEventName && hasStartDate;
  }, [formState.teamMember, formState.eventType, formState.customEventName, formState.startDate, formState.startTime, formState.endTime, isNormalSchedule]);
  ...
  const formControlRenderers: { [key: string]: (customProps?: Record<string, any>) => React.ReactElement | null } = {
    smartEventInput: (customProps = {}) => (
      <SmartEventInput 
        ...
        onClear={() => {
          ...
          // Set internal state AND notify parent with the same values
          setFormState(clearedValues);
          if (onChange) {
            onChange(clearedValues); // Pass the actual cleared values, not {}
          }
        }}
        {...customProps}
      />
    ...
    recurrence: (customProps = {}) => (
      <RecurrenceSelectMenu
        ...
        excludeEveryWeek={isNormalSchedule} // Hide "Every Week" for normal schedules since it's redundant
        selectMenuProps={{
          isSearchable: true,
          styles: SelectMenuStylePresets.Large,
